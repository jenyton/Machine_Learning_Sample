...
def  print_age_result(word,result):
    matching=[a for a in result if word in a]
    suitable=[s for s in matching if "Yes" in s]
    print "************** Suitable Tourist Place for CHILDREN ************** \n"
    if suitable:
        for m in suitable:
            r=m.replace('IF Age group EQUALS '+word,'')
            r=r.replace(' AND','')
            r=r.replace('EQUALS','-')
            r=r.replace('THEN Yes','')
            if r !=' ':
                print r
            else:
                print "There is no restriction in tourist place. \nCan visit all kind of tourist places"	
    else:
        print "There is no Suitable Tourist place for these age group"

    print "\n\n"
    non_suitable=[ns for ns in matching if "No" in ns]
    print "************** Non Suitable Tourist Place for CHILDREN ************** \n"
    if non_suitable:
        for m in non_suitable:
            r=m.replace('IF Age group EQUALS '+word,'')
            r=r.replace(' AND','')
            r=r.replace('EQUALS','-')
            r=r.replace('THEN No','')
        ...
        print "There is no non-Suitable Tourist place for these age group"
    print "\n\n"



def age_result(result):
	
    age= raw_input("Enter the age group \n1.Youth (18 to 25)\n2.DINKS (Double Income No Kids)\n3.SINKS(Single Income No Kids)\n4.EMPTY NESTERS(45 to 60)\n5.BOOMERS(Members with kid)\n6.CHILDREN\n")
    if age == '1'  :
        print_age_result('YOUTH',result)
    elif age == '2':
        print_age_result('DINKS',result)
    elif age == '3':
        print_age_result('SINKS',result)
    elif age == '4':
        print_age_result('EMPTY NESTERS',result)
    elif age == '5':
        print_age_result('BOOMERS',result)
    elif age == '6':
        print_age_result('CHILDREN',result)	
